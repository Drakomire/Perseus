...
class Stats:
    ...
    def getOilCostAtLevel(ship):
      #Submarines use a different oil cost equation than different hull classes
      max_cost = Stats.getMaxOilCost(ship)
      if (SHIP_LOCATION[ship.hull_id] == "Submarine"):
        return math.floor((max_cost+1)*math.min(ship.level,99)/200)
      else:
        return math.floor(max_cost*math.min(ship.level,99)/200)+1
